...
To enable IDPs(Identity Providers) on your Daytona installation, we utilise an open-source identity and access management application [Keycloak](https://www.Keycloak.org/) to add multiple different identities providers. It will allow you to log into your Daytona dashboard via different IDPs, create [workspaces](/usage/workspace) from different Git providers, and manage access scope, permissions, sessions, and events. Also, monitor and manage users within your organization with Daytona.
...
To configure and manage your identity providers, you will need to fulfill the below prerequisites.  

- **Daytona Installation**: To install Daytona on your infrastructure, you can [download Daytona from this link](https://github.com/daytonaio/installer). Upon every Daytona installation, you will have a URL to access your Daytona dashboard. You can then access your Daytona installation's identity and access management interface via its subdomain `id`.  When you are logging into KeyCloack on `Adminstration Console`, you will be asked for `username or email` and `password` as credentials. You can only obtain this information during your [Daytona installation](https://github.com/daytonaio/installer). 
...
In this section, we will guide you to configure Github as an identity provider for your Daytona Installation. This guide is only suitable only if you didn't use Github as an identity provider during the initial installation stage(when you install Daytona on your infrastructure). 
...
6. **Advance Settings on Keycloak**: On Keycloak's Provider details page, copy and paste below access scope into the field `Scopes` under the `Advanced settings`:

```text
read:user user:email repo
```
> - These three scopes will allow users to log in via email, read GitHub users' information, and access logged-in users' repositories to gain read, write, and admin rights. 
...
> - If you want to type in scopes by yourself, remember to use <Keyboard shortcuts="Space ␣" /> to separate different scopes but not comma <Keyboard shortcuts="," />.
...
_At this stage, you have successfully configured all the necessary information to enable Github as your identity provider for your Daytona installation. If you open your Daytona URL, you should see the button `Github` on the login page. If not, scan through the table above and confirm if all the inputting information is correct._
...
3. **Default realm**: After you log in, make sure you are on the `default` realm. You should see the homepage show `Welcome to default` at the center of the screen. If not(the home page shows scattered information with `Server infor` with the title `master realm`), you can select `default` under the dropdown menu on the top left side of the home page. 
...
5. **Configure OAuth consumer**: Navigate back to BitBucket, on the `Add OAuth consumer` page, give your application a distinguishable name in the `Name` field(required, something like `daytona-domain-name`), then paste the `Redirect URI` from the previous step to `Callback URL`. The `URL` field would be your Daytona dashboard URL. You can leave the `Privacy policy URL` and `End user license agreement URL` empty and don't toggle `This is a private consumer`(Your authorization will fail if you toggle this). 
...
_These permissions will allow Daytona to authenticate your BitBucket account during account creation and log-in process,  access your workspace membership, find team members, and access your repositories to create workspace whenever you are using Daytona._
