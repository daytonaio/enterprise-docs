...
1. Click the bin icon next to the Identity Provider to remove it.
    ...
    For security purposes, removing an Identity Provider requires you to confirm the action.

2. Click **`Delete`** to confirm the removal of the Identity Provider.
...
Before adding Azure DevOps as an Identity Provider, register an Azure DevOps OAuth client to enable users to authenticate to Daytona using their Azure DevOps account.

### Azure DevOps OAuth Client
...
7. Copy the `Client ID`, `Client Secret`, and `Tenant ID` values to use when configuring the Azure DevOps Identity Provider.
...
Before adding Bitbucket as an Identity Provider, register a Bitbucket OAuth client to enable users to authenticate to Daytona using their Bitbucket account.

### Bitbucket OAuth Client
...
3. Enter the details for the OAuth App.
    ...
    - `Callback URL` must be set to the `Redirect URI` value found in the form while adding Bitbucket as an Identity Provider.
...
5. Copy the `Client ID` and `Client Secret` values to use when configuring the Bitbucket Identity Provider.
...
Before adding Bitbucket Server as an Identity Provider, register a Bitbucket Server OAuth client to enable users to authenticate to Daytona using their Bitbucket Server account.

### Bitbucket Server OAuth Client
...
6. Enter the details for the OAuth App.
    ...
    - `Application details` must be set to the `Redirect URI` value found in the form while adding Bitbucket Server as an Identity Provider.
...
8. Copy the `Client ID` and `Client Secret` values to use when configuring the Bitbucket Server Identity Provider.
...
Before adding GitHub as an Identity Provider, register an OAuth client to enable users to authenticate to Daytona using their GitHub account.
...
3. Enter the details for the OAuth App.
    ...
    - `Authorization callback URL` must be set to the `Redirect URI` value found in the form while adding GitHub as an Identity Provider.

4. Click **`Register application`** to create the GitHub OAuth App.
5. Copy the `Client ID` and `Client Secret` values to use when configuring the GitHub Identity Provider.
...
Before adding GitHub Enterprise Server as an Identity Provider, register a GitHub Enterprise Server OAuth client to enable users to authenticate to Daytona using their GitHub Enterprise Server account.

### GitHub Enterprise Server OAuth Client
...
3. Enter the details for the OAuth App.
    ...
    - `Authorization callback URL` must be set to the `Redirect URI` value found in the form while adding GitHub Enterprise Server as an Identity Provider.

4. Click **`Register application`** to create the GitHub Enterprise Server OAuth App.
5. Copy the `Client ID` and `Client Secret` values to use when configuring the GitHub Enterprise Server Identity Provider.
...
Before adding GitLab as an Identity Provider, register a GitLab OAuth client to enable users to authenticate to Daytona using their GitLab account.
...
5. Copy the `Client ID` and `Client Secret` values to use when configuring the GitLab Identity Provider.
...
Before adding GitLab Self-Managed as an Identity Provider, register a GitLab Self-Managed OAuth client to enable users to authenticate to Daytona using their GitLab Self-Managed account.

### GitLab Self-Managed OAuth Client
...
5. Copy the `Client ID` and `Client Secret` values to use when configuring the GitLab Self-Managed Identity Provider.
...
Before adding Google as an Identity Provider, register a Google OAuth client to enable users to authenticate to Daytona using their Google account.
...
10. Enter the details for the OAuth App.
    ...
    - `Authorized redirect URIs` must be set to the `Redirect URI` value found in the form while adding Google as an Identity Provider.

11. Click **`CREATE`** to create the Google OAuth App.
12. Copy the `Client ID` and `Client Secret` values to use when configuring the Google Identity Provider.
...
Before adding Microsoft as an Identity Provider, register a Microsoft OAuth client to enable users to authenticate to Daytona using their Microsoft account.
...
Before adding OpenID Connect as an Identity Provider, register an OpenID Connect OAuth client to enable users to authenticate to Daytona using their OpenID Connect account.