...
4. Host the Sample repository publicly on a Git repository or another platform.
...
4. Enter the public raw URL of the **`index.json`** file in the **`Workspace samples`** field. 

```text
https://raw.githubusercontent.com/<username>/<repository>/<path>/index.json
```

Samples Index will be used to display the available Samples when creating a Workspace.
...
Daytona Samples are integrated via Samples Index (`index.json`) file that serves as a registry of available Daytona Samples for creating Workspaces.

Enter the public raw URL of the **`index.json`** file in the **`Workspace samples`** field:

```text
https://raw.githubusercontent.com/daytonaio/samples-index/refs/heads/main/index.json
```

Samples Index will be used to display the available Daytona Samples when creating a Workspace. 

The chosen Sample will be cloned and used to create the Workspace. Daytona Sample repository will serve as the core content of the Sample used to create a Workspace.