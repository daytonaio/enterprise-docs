...
The default samples are be available while [creating a Workspace](/usage/workspaces). The sample will be cloned and used to create the Workspace.
...
View examples of custom sample structures in the Daytona [samples repositories](https://github.com/orgs/daytonaio/repositories?q=sample).

2. Host the custom samples publicly on a Git Provider or another platform.

3. Create a JSON file with an array of objects representing the samples.
...
4. Host the custom samples JSON file publicly on a Git Provider or another platform.

After creating the custom samples and their corresponding JSON files, you can use the samples to create a Workspace. Follow the guide below to [use samples](#use-samples) in Daytona.
...
Daytona provides an option to use [default samples](#daytona-samples) or [custom samples](#custom-samples) when creating a Workspace.

1. Enter the raw URL of the custom sample JSON file in the **`Workspace samples`** field.
...
The custom samples will be available while [creating a Workspace](/usage/workspaces). The sample will be cloned and used to create the Workspace. The repository will serve as the core content of your sample and will be used to build your Workspace.
