...
- **Granting access to Git provider's organization access**: Organization members can always request that an owner approve access to organization resources for Daytona, and organization owners receive notification of pending requests. After additional organizations are authenticated, you can create workspaces based on repositories in organizations.
...
- Open your terminal and paste the text below, replacing the email used in the example with the same email address you used with the Identity Provider for your Daytona installation.
