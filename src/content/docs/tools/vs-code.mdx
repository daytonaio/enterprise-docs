...
- **Repository isolation**: Each repositories created under the `WORKSPACES` tab are in their standalone, isolated environment. 
...
### 3. Managing teams
Repositories are separated between teams. Repositories created within a team only exists in that team, if you create a new team the repositories might appear empty. 
...
#### 4. Managing profiles
- **Checking profile**: Under the `PROFILES` section within the extension, you can check your existing profile. Under each profile
- **Selecting profile**: Hover over your profile's name and choose `Select`, when you choose a new profile, commit to test profile
