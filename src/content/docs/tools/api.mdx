...
API requests must be encoded as JSON with the `Content-Type: application/json` header. If not otherwise specified (some are `*/*`), responses from your Daytona API, including errors, are encoded as JSON as well. As these settings are injected in Swagger documentation automatically, no further configuration are needed. 
