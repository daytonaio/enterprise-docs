...
Daytona's JetBrains Gateway plugin allows you to connect to your Daytona workspace from JetBrains Gateways, an application purpose-built for a cloud-based development environment without even downloading the selected JetBrains IDE to drastically enhance your development experience. 
...
The installation process is split into two parts. The first is the package installation, and the second part is how you established the connection between your [Daytona installation](#prerequisites) and JetBrains Gateway. You will have to install the Daytona Gateway Plugin Package.


1. **Daytona's JetBrains Plugin Installation**: Click on this link [Daytona Gateway Plugin Package](https://download.daytona.io/jetbrains/gateway/latest/jetbrains-gateway-daytona-plugin-latest.jar) to download the latest package(if the download didn't start, navigate to [this link](https://download.daytona.io/) to find the latest JetBrains Gateway plugin version: Home page > JetBrains > gateway > latest). The download will start automatically. <u>Remember the location, or directory of where the package is downloaded to, as you will need it later.</u>
...
5. **Finish installation**: JetBrains Gateway will navigate you back to the `Plugins` page. You will see the Daytona Gateway Plugin Package is installed (under the search bar `/downloaded`). Click on the blue `OK` button at the bottom right to close the plugin window. 
...
3. **Open Code Editor**
- After you select `Open`, your default code editor will already be selected on the pop-up window with the title **Open \<your-selected-workspace-id\>**. Although you can still change it to another JetBrains IDE here (You may choose other options like Theia and VS Code, but we will need one of the JetBrains IDEs in this guide). And then click on the green button `Open` at the bottom right corner of the pop-up window. 

- After you click on the `Open` button, it will have a new pop-up window shows the instructions on how to use Daytona's JetBrains Gateway Plugin, with the title **Open workspace in a JetBrains IDE**. Since you are reading this guide, you can click on the green button `Open` at the bottom again to skip the instruction.
...
- Note that a new browser tab will also open, indicating that this workspace has started. When you click on the green button `Open in WebStorm`, it will redirect you to the JetBrains Gateway application, but not to open the code editor.
...
- You will see a pop-up window with two input fields: `Name`and `Base Api URL`. In the `Name` field, you can name your profile as you wish(Recommend using your domain name as you might have multiple Daytona installations). In the `Base Api URL` field, you will need to enter your Daytona dashboard URL(kept from [prerequisites](#prerequisites), like `https://your-domain-name`). After you enter the URL, click on the green button `New` at the bottom left corner of the pop-up window. 
...
- On your local machine, open your terminal and type in the following command to find your public key:
  ...
  ls ~/.ssh/*.pub
  ```
- This command will return the path of your public key. Copy the path, might in the format of `~/.ssh/id_rsa.pub` or `~/.ssh/id_ed25519.pub`. Then within the terminal, type in the following command to find your public key: 
...
- On your Daytona dashboard, click on the `SSH Keys` on the left-hand side menu. At the **SSH Keys** section, click on the <Icon icon="jetbrains-add" class="icon-display" /> `Add` button. A pop-up window with two input fields and one selection field will show up: 
...
- A pop-up window will open in the browser with the title `Connect to \<your-workspace-name\>`. You will see two tabs that carry information that are available to copy paste. Select the `Public Key` tab and copy the information under the `Public Key` section, save it somewhere safe. The format will look like 
...
_After these steps, your Daytona installation with JetBrains Gateway is established. you will be navigated to Daytona's configuration page on JetBrains Gateway, which includes your team name, GitHub username etc. We will talk about them in the next section._

## Features

Now that you have established the connection between your Daytona installation and JetBrains Gateway, you can navigate to Daytona's configuration page by clicking on the `Connect` button under Daytona on the welcome page. In this section, we will walk you through the features of Daytona's JetBrains Gateway Plugin, including how to create, manage, and use your [workspace](/usage/workspaces), how to manage team, profile information and more.

### Creating workspaces

Each [workspace](/usage/workspaces) created on Daytona is isolated in their own environment, even if it came from the same repository. In the following two sub-sections, we will talk about how to create and manage your workspaces with Daytona in JetBrains Gateway. Workspaces created within the JetBrains Gateway via the Daytona plugin are synced with your Daytona installation. 
...
- 2. **Create via dropdown menu**: On the right side of the field `Repository:`, click on the dropdown menu and select a repository, then click on `New Workspace` to create a workspace. Note that <u>it might take a couple of seconds to create and start a workspace until it is loaded, appearing under `Recent Workspaces`</u>. If you click on it multiple times while the workspace is creating, or starting, you might <u>accidentally create multiple workspace</u> with the same repository.
...
- 3. **Live preview link**: If you are working on a web application(includes a port), you can run `npm run dev`(or other commands) to start the local development server. Once the server is up and running, you can either click on the URL that shows up in the terminal to see the application locally(you can only see live change if your application enabled hot module replacement([HMR](https://webpack.js.org/concepts/hot-module-replacement/)), if not, you can only see changes after restarting the server).

{/*TODO: This feature haven't yet built, put this back after it is built*/}
...
- 1. Click on the <Icon icon="jetbrains-add-profile" class="icon-display"/> button next to the <Icon icon="jetbrains-edit" class="icon-display"/>. On the pop-up window, you will need to input `Name`(profile name being displayed on Daytona's JetBrains Gateway configuration page, recommend using a domain name) and `Base Api URL`(your Daytona dashboard URL, no subdomain required. For example: if your Daytona dashboard URL is `https://daytona.com`, the input for this field would be `daytona.com`). 

- 2. After inputting the information, click on `New`. You will then be redirected to your Daytona dashboard's signin page to select an identity provider to log in. Depending on whether you have a session that exists(that you are logged in) with your identity provider, you may be redirected to your identity provider(Github, Gitlab, BitBucket) to sign in. 
...
At the moment, you can only manage team-related information on your Daytona dashboard. Note that your workspaces are correlated to your team. If you switch to another team (say Team B), you won't be seeing workspaces created on previous team(say Team A).
